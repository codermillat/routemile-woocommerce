...
- **Test runner:** `php tests/RouteWTestRunner.php` → must report **323/323 pass** before any commit (includes Leaflet SHA-256 integrity, the DS1–DS16 design-system suite, the PG1 payment-gate suite, and the CB1 checkout-block suite)
...
Expected: `Passed: 323, Failed: 0`. The runner checks:

- File structure (all required files exist, including the checkout split files + bundled Leaflet + Mile Zero build sources)
- Bundled vendor integrity (SHA-256 of the bundled Leaflet 1.9.4 JS/CSS matches the pinned hashes)
- Code quality (no unlimited queries, nonce verification on all AJAX files)
- Plugin headers (WordPress + WooCommerce; `Tested up to` in readme.txt only, never in the PHP header — WP.org review directive 2026-09-04)
- Hooks & filters (registered correctly, custom statuses registered, H4/M2+M3 lifecycle wiring)
- Security patterns (ABSPATH check, nonce verification)
- Design system DS1–DS16 + payment-gate PG1 suites

**Run the test runner after every change. It must report 323/323 before commit.**
...
## 13. WordPress.org Directory compliance (APPROVED 2026-09-06 as `routemile-for-woocommerce` v1.6.3)

> The FoodXpress → RouteMile rename (v1.5.0) resolved the reviewer's trademark/similarity flag. Every bullet below was raised in the `AUTOPREREVIEW` round (Review ID `P0TDX359449HGN`, 2026-08-26) or a follow-up round, fixed, and approved. Keep each invariant green — Plugin Check (`0 errors, 0 warnings` on the shipped zip) plus the test runner pin them.

| # | Review rule | Invariant (MUST hold) | Anchor | Pinned by |
|---|---|---|---|---|
| 1 | Distinctive name/slug, no trademark confusion | Display name `RouteMile for WooCommerce`, slug + text domain `routemile-for-woocommerce` everywhere; no `foodxpress` runtime strings (only the intentional `fxw_*` legacy-migration map in `routemile-woocommerce.php`) | `routemile-woocommerce.php:3,11`, `readme.txt:1` | DS10 text-domain test |
| 2 | `wp_enqueue_*` only, no raw inline script/style | All CSS/JS via `wp_register/enqueue_*` (+ `wp_print_styles/scripts` on the standalone receipt page, which has no `wp_head` cycle); no raw `<script>`/`<style>` tags in `includes/` or `templates/` | `templates/receipt-template.php:145-152` | Code-quality scan |
| 3 | Nonces + capabilities on every state change | Per-handler `wp_verify_nonce()` (or `check_ajax_referer()`) AND `current_user_can()` on every form-POST/AJAX handler. **Intentional exception:** the WC Settings tab relies on WooCommerce's own settings-tab nonce (`includes/class-routew-settings.php:7-10`) — adding ours would fail every legitimate save | `class-routew-dashboard-actions.php:44-48`, `class-routew-delivery-boy-view.php:124-128`, `class-routew-admin-bar.php:92-96` | AJAX-security scan |
| 4 | Escape late, context-correct | `esc_html()` / `esc_attr()` / `esc_url()` / `wp_kses_post()` at output; `wp_unslash()` before `sanitize_*()` on input; strict `===` auth checks; HPOS CRUD only (`wc_get_order`, `get_meta`/`update_meta_data`) | `templates/receipt-template.php:133-137`, `templates/emails/routew-order-status.php:25` | Security-patterns scan |
| 5 | Prefixes ≥4 chars, distinct | `routew_` (6) / `ROUTEW_` / `routew-` for functions, classes, options, transients, meta keys, AJAX actions, slugs; template locals prefixed (`$routew_*`) | Whole tree | WPCS `PrefixAllGlobals` |
| 6 | Attribution opt-in (Guideline 10) | Map-provider "Powered by" credit renders ONLY when `routew_show_map_credit` is ticked; default OFF; readme states the OFF-default + license responsibility | `class-routew-settings-maps.php:169-202`, `class-routew-checkout-maps.php:137-145`, `readme.txt:62` | Manual (no test — verify on sight) |
| 7 | Short description ≤150 chars | `readme.txt:11` short desc stays ≤150 chars (currently 145) | `readme.txt:11` | Manual (count before release) |
| 8 | Ship zip hygiene | `.distignore` excludes `.wordpress-org/`, `tests/`, `tools/`, `docs/`, `skills/`, `design-assets/`, `audit-e2e/`, `e2e-audit/`, `.playwright-mcp/`, QA `*.png`, VCS/IDE files; `LICENSE.md` is intentionally KEPT in the archive | `.distignore` | Manual (`wp dist-archive --dry-run`) |
| 9 | External-services disclosure (Guideline 6) | `readme.txt` "Privacy & external services" lists every provider (Google, OSM tiles, Nominatim + 1 req/s policy, OSRM, MapTiler, Geoapify) with what data is sent + when + service/Terms/Privacy links, plus the no-telemetry statement | `readme.txt:51-64` | Manual (re-check URLs per release) |
| 10 | No high top-level admin menu (Guideline 11) | Deliveries lives ONLY as `add_submenu_page('woocommerce', …)` — never `add_menu_page`; legacy top-level URL 301-redirects one cycle | `class-routew-dashboard-render.php:44-62` | Manual (no test — grep before release) |
| 11 | Headers: `Tested up to` readme-only; WC headers; HPOS | `Tested up to` in `readme.txt` ONLY (never in the PHP header — reviewer explicit ask); `Requires Plugins: woocommerce`, `WC requires at least`, `WC tested up to` in the PHP header; HPOS `declare_compatibility('custom_order_tables')` | `readme.txt:5`, `routemile-woocommerce.php:13-17,54-65` | `testPluginHeaders` |
| 12 | Version parity | `Stable tag` (readme) === `Version` header === `ROUTEW_VERSION` on every release | `readme.txt:7`, `routemile-woocommerce.php:6,26` | Release checklist |

**Pre-release checklist (every version bump):** short-desc char count → external-service URLs live → submenu-only grep → `wp dist-archive --dry-run` file list → Plugin Check 0/0 → `php tests/RouteWTestRunner.php` → `323/323` → bump all three version sites together.

---

*End of AGENTS.md. Last updated 2026-09-10 for v1.6.3 (WP.org approval ingest: §7 test counts 278→312, new §13 directory-compliance ledger, `.distignore` audit-dump coverage).*
