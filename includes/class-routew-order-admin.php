	...
	public function handle_order_actions()
	{
		if (isset($_GET['routew_action']) && isset($_GET['order_id']) && isset($_GET['_wpnonce'])) {
			...
			switch (sanitize_text_field(wp_unslash($_GET['routew_action']))) {
				case 'reject':
					$order->update_status('cancelled', __('Order rejected by admin.', 'routemile-woocommerce'));
					break;
case 'reassign':
						...
						$order->update_status($revert_to, __('Delivery boy has been unassigned — order returned to kitchen.', 'routemile-woocommerce'));
						$order->save();
						break;
			}
	...
	public function add_delivery_meta_box()
	{
		...
		foreach ($screens as $screen) {
			add_meta_box(
				...
				__('RouteMile Delivery', 'routemile-woocommerce'),
	...
	public function render_delivery_meta_box($post_or_order)
	{
		...
		<?php if ($delivery_address): ?>
			<div style="margin-bottom: 15px; padding: 10px; background: #f9f9f9; border-left: 4px solid #0073aa;">
				<p><strong><?php esc_html_e('Delivery Address:', 'routemile-woocommerce'); ?></strong><br>
					<?php echo esc_html($delivery_address); ?></p>

				<?php if ($delivery_distance): ?>
					<p><strong><?php esc_html_e('Delivery Distance:', 'routemile-woocommerce'); ?></strong>
						<?php echo esc_html($delivery_distance); ?> km</p>
				<?php endif; ?>

				<?php if ($delivery_lat && $delivery_lng): ?>
					<p><strong><?php esc_html_e('Coordinates:', 'routemile-woocommerce'); ?></strong>
						<?php echo esc_html($delivery_lat); ?>, <?php echo esc_html($delivery_lng); ?></p>
				<?php endif; ?>
			</div>
		<?php elseif ($unit): ?>
			<p><strong><?php esc_html_e('Flat/House/Unit:', 'routemile-woocommerce'); ?></strong> <?php echo esc_html($unit); ?></p>
		<?php endif; ?>
		<p>
			<strong><?php esc_html_e('Payment Method:', 'routemile-woocommerce'); ?></strong><br>
			<?php echo esc_html($payment_method); ?>
			<?php if ('cod' === $order->get_payment_method()): ?>
				<br><strong><?php esc_html_e('Amount to Collect:', 'routemile-woocommerce'); ?></strong>
				<?php echo wp_kses_post($order->get_formatted_order_total()); ?>
			<?php endif; ?>
		</p>
		<p>
			<label for="routew_delivery_boy_id"><?php esc_html_e('Assign Delivery Boy', 'routemile-woocommerce'); ?></label>
			<?php if (!empty($delivery_boys)): ?>
				<select name="routew_delivery_boy_id" id="routew_delivery_boy_id" style="width:100%;">
					<option value=""><?php esc_html_e('Select a Delivery Boy', 'routemile-woocommerce'); ?></option>
			...
			<?php else: ?>
			<p><?php esc_html_e('No delivery boys found. Please create a user with the "Delivery Boy" role.', 'routemile-woocommerce'); ?></p>
		<?php endif; ?>
		</p>
		<p>
			<?php
			...
			<a href="<?php echo esc_url(wp_nonce_url(add_query_arg('routew_print_receipt', $order_id, site_url()), 'routew_print_receipt')); ?>"
				target="_blank" class="button"><?php esc_html_e('Print Receipt', 'routemile-woocommerce'); ?></a>
		</p>
		<p>
			<?php
		...
		if (!$edit_link) {
			...
			<a href="<?php echo esc_url(wp_nonce_url(add_query_arg('routew_action', 'reject', $edit_link), 'routew_order_action')); ?>"
				class="button button-danger"><?php esc_html_e('Reject Order', 'routemile-woocommerce'); ?></a>
			<a href="<?php echo esc_url(wp_nonce_url(add_query_arg('routew_action', 'reassign', $edit_link), 'routew_order_action')); ?>"
				class="button"><?php esc_html_e('Re-assign', 'routemile-woocommerce'); ?></a>
		</p>
		<?php
	}
...
// rendered in the meta box; legacy meta is preserved in WC meta store.)
		...
		if (isset($_POST['routew_delivery_boy_id'])) {
			...
			if ($new_id && $new_id !== $prev_id) {
				...
				if ($auto_assign) {
					$order->update_status('routew-assigned', __('Order assigned to delivery boy.', 'routemile-woocommerce'));
				} else {
					$order->add_order_note(__('Delivery boy assigned (status unchanged).', 'routemile-woocommerce'));
				}
			}
		} else {
			$order->save();
