...
## [1.6.4] — 2026-09-10 — Mile Zero design system

### Security (2026-09 audit: unpaid orders can no longer be forged to "paid")
- **Payment-completion gate.** WooCommerce automatically marks any order moved to `completed` as paid (`maybe_set_date_paid`). Previously a shop manager could mark an unpaid order completed from the deliveries dashboard, and — worse — an assigned **delivery rider** could do the same through the rider app on any prepaid order (BACS/cheque): two button taps turned an unpaid order into `paid=1` with no transaction record. All four manual transition paths (manager admin-post + AJAX, rider admin-post + AJAX) now call `ROUTEW_Order_Lifecycle::can_complete_order()`, which refuses the `completed` transition while `needs_payment` holds. Gateway-paid orders, COD with confirmed cash collection (the rider's cash-confirm already sets `date_paid`), and zero-total orders pass unchanged — the legitimate pick_up → cash → deliver flow is untouched. Regression suite **PG1** (8 tests) pins the gate, all four call sites, the COD sequence, and scans the transition files so any future completed-capable action that forgets the gate fails the build.
- **REST CSRF re-audit:** the checkout REST endpoints (`cart-update`, `validate-location`, `settings`, `geocode`) were tested end-to-end for cross-origin abuse. WordPress core demotes cookie-authenticated nonce-less REST requests to anonymous rather than rejecting them, but the WooCommerce session cookie ships without `SameSite` (browser default Lax), which keeps the victim's session off cross-site POSTs — a live cross-origin form-POST experiment confirmed the victim's session and cart stay untouched. The `__return_true` permission callbacks were therefore kept (reverting an earlier over-strict attempt that would have broken guest checkout); anonymous abuse remains bounded by the per-IP rate limiter.

### Fixed (block checkout live-session 2026-09-10 — "No shipping options" report)
- **Fresh-quote distance regression.** The toast/totals fee-unification refactor stopped assigning `$distance_in_km` on the fresh-quote path in `ROUTEW_Shipping_Method::calculate_shipping()` — silently skipping the delivery-radius check and misquoting whenever the pin moved past the toast quote. The assignment is restored; toast and totals agree on every pin (verified live: 9.22৳ quoted = 9.22৳ charged).
- **WC 11 field-registration timing.** Additional checkout fields registered on `woocommerce_blocks_loaded`, which WC 11 fires before `after_setup_theme` — every checkout render logged "register on woocommerce_init or later" + early-translation notices. Registration now hooks `woocommerce_init` (the wrapper's own deferral keeps it exactly-once on every WC version), and the disallowed `placeholder` attributes (silently dropped by WC 8.6+ with a notice) are removed. Checkout renders notice-free.
- **Silent stale totals on transient extension failures.** The pickers fired `extensionCartUpdate` fire-and-forget, so a rejected promise (flaky network, racing cart update) left "No shipping options" on screen with no error and no retry. Both pickers (`checkout.js`, `checkout-leaflet.js`) now retry once after 1.5s and warn to the console.
- Regression suite **CB1** (4 tests) pins the hook, the attribute allowlist, the km assignment, and the retry in both pickers. Suite now **316/316**.

### Fixed (classic thank-you scope 2026-09-10)
- **Classic `/checkout/order-received/` was completely unstyled.** The design-system scope reached the blockified thank-you (per-block classes) but the classic shortcode thank-you had no `.routew-ui` ancestor at all — the tracking stepper icons rendered at raw intrinsic size (hundreds of px tall) and the overview strip, details table and address cards fell back to unstyled theme markup (caught live on order #527). `woocommerce_before_thankyou` / `woocommerce_thankyou` now open/close the shared `routew-ui routew-account routew-account--thankyou` wrapper — gated by `has_block('woocommerce/checkout')` so the blockified template (where those hooks fire inside sibling blocks) is untouched.
- **Classic checkout stuck on "No shipping options" after the pin.** WooCommerce caches rates per package-hash and the pin is not part of it: a recalc that ran before the pin existed cached EMPTY rates, and every later recalc served the stale set without ever running the method again (toast quoted 5.68 while totals stayed empty). `ROUTEW_Checkout_Handler::stamp_pin_on_packages()` (via `woocommerce_cart_shipping_packages`) now stamps the session pin onto each package, so moving the pin changes the hash and forces a fresh calculation on classic and block flows alike. Verified live: 5.68 → move pin → 12.76, toast == totals both times.
- CB1 grows to 7 tests; suite now **319/319**.

### Fixed (map honesty 2026-09-10 — restaurant pin + true coverage)
- **Fixed restaurant marker: two pins now.** Both pickers (Google + Leaflet) render a second, non-draggable ink-badge marker at the restaurant with a name tooltip (receipt name → site title), layered under the blue customer pin. Asset-free (inline SVG divIcon / data-URL). Step-1 copy now reads "drag the delivery pin … The store marker never moves."
- **Coverage circle tells the truth.** The circle used the raw 10 km radius while the zone rule compares ROAD distance — pins inside the circle but across a river routed far and got rejected with a bare "we do not deliver", reading as a contradiction. The circle is now drawn at radius ÷ road-factor (7.69 km here): mathematically exact for estimate providers, a conservative guide for true-road ones (`circle_exact` flag), dashed on Leaflet to signal approximation.
- **Rejections name the road distance.** All three zone gates (REST toast, classic + blocks validation) share `ROUTEW_Checkout_Handler::out_of_zone_message()`: "…it is 51.20 km by road and we deliver within 10.00 km of the restaurant."
- CB1 grows to 10 tests; suite now **322/322**.

### Fixed (theme compatibility 2026-09-10 — Astra + block checkout)
- **Block checkout under a classic theme had no map picker.** The picker injected via `render_block_core/post-content`, which never fires on classic themes (Astra, Storefront, …) — a block checkout there rendered with no location step at all (proven live on Astra). A `the_content` fallback (priority 8, main loop only) now covers classic themes, sharing an exactly-once flag with the block-theme filter so block themes still render a single picker. Verified end-to-end on Astra: picker renders, pin validates, rate totals match.
- **WC tested up to 11.1** (header bump; all flows verified live on WC 11.1.0 + WP 7.1 + Astra 4.13).
- CB1 grows to 11 tests; suite now **323/323**.
